...
let unfundedGamesCtn = GAMES_JSON.filter(game =>  game.pledged < game.goal).length

let unfundedGames = GAMES_JSON
    .filter(game =>  game.pledged < game.goal)
    .reduce((sum, game) => sum + game.pledged, 0);
console.log(unfundedGames)

let fundedGamesCtn = GAMES_JSON.filter(game =>  game.pledged > 0).length
let fundedGames = GAMES_JSON
    .filter(game =>  game.pledged > 0)
    .reduce((sum, game) => sum + game.pledged, 0);
console.log(fundedGames)

// create a string that explains the number of unfunded games using the ternary operator
let unfundedGamesStr = `A total of $${fundedGames.toLocaleString()} has been raised for ${fundedGamesCtn} games. Currently, ${unfundedGamesCtn > 1 ? unfundedGamesCtn+" games" : unfundedGamesCtn+" game"} remains unfunded. We need your help to fund these amazing games!`
console.log(unfundedGamesStr)

// create a new DOM element containing the template string and append it to the description container
const templateStrContainer = document.createElement('p');
templateStrContainer.innerHTML = unfundedGamesStr
templateStrContainer.classList.add('template-str');
descriptionContainer.appendChild(templateStrContainer)
